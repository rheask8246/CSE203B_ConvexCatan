...
- **Placement budget per player** (initial phase):
  ...
  In mid-game, this is relaxed to \(\sum_{l \in L} x_{pl} \le \text{budget}_p\),
  where the budget depends on how many settlements/cities a player already has.
...
- **Distance constraint (relaxed)**:
  ...
  the “no adjacent settlements” rule in relaxed form:
